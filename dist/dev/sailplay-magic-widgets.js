	    ...
	    return function (scope, elm, attrs) {
	      ...
	      scope.setTag = function (phone, tag, parent_phone) {
	        ...
	        SAILPLAY.jsonp.get(_config.DOMAIN + tagDelete, tag_obj_delete, function () {
	          SAILPLAY.jsonp.get(_config.DOMAIN + tagAdd, tag_obj);
	          if (tag == 'booked') SAILPLAY.jsonp.get(_config.DOMAIN + tagAdd, parent_tag);
	        });
	      };
	                ...
	                SAILPLAY.jsonp.get(_config.DOMAIN + tagExist, tag_obj, function (res) {
	                  angular.forEach(res.tags, function (tag) {
	                    ...
	                    lead.partners = lead.partners.filter(function (obj) {
	                      return ['closed', 'booked'].indexOf(obj.tag) == -1;
	                    });
	                    scope.$digest();
	                  });
	                });
	...
	var ProfileWidget = {
	  ...
	  controller: function controller($rootScope) {
	    ...
	    return function (scope, elm, attrs) {
	      ...
	      $rootScope.$on('openProfileForce', function () {
	        scope.profile.show_fill_profile = true;
	        $rootScope.profile_fill_force = true;
	      });
	      scope.profile = {
	        history: false,
